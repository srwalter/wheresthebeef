...
parameters from the function are rendered as tables.

clearUI()

Remove any previously generated UI elements

callProcedureClear(proc_name)

Lazy-mode helper that combines clearUI() and callProcedure()

callProcedureChained(proc_name, next_proc)
...
callProcedureEditDelete(proc_name, url, edit_proc, delete_proc)
...
callProcedureLinks(proc_name, url, links)

This is a more general version of callProcedureEditDelete.  links is a
dictionary, where keys are link labels and values are the stored procedure
names.  The value can also specify if the procedure should be automatically run
instead of requiring the user to click the button.
